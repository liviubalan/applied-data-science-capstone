...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        title_current = 'Entered site {0}'.format(entered_site)
        fig = px.pie(spacex_df, values='class', names='Launch Site', title=title_current)
        return fig
    else:
        # return the outcomes piechart for a selected site
        filter = spacex_df[spacex_df['Launch Site'] == entered_site]
        filter_2 = filter.groupby(['Launch Site','class']).size().reset_index(name='class count')
        title_current = 'Entered site {0}'.format(entered_site)
        fig = px.pie(filter_2, values='class count', names='class', title=title_current)
        return fig
# TASK 4:
